...
object Huffman {
  ...
  /**
    ...
    */
  def until(pred: List[CodeTree] => Boolean, merge: List[CodeTree] => List[CodeTree]) (trees: List[CodeTree]): List[CodeTree] = {
    if (pred(trees))
      trees
    else
      until(pred, combine)(combine(trees))
  }
      ...
      subtree match {
        case Leaf(char, _) => {
          bits match {
            ...
            case x :: xs => loop(tree, tree, bits, acc ::: List(char))
          }
        }
        case Fork(l, r, _,_) => {
          bits match {
  ...
  def convert(tree: CodeTree): CodeTable = {
    def loop(tree: CodeTree, bitAcc: List[Bit], tableAcc: CodeTable
      ): CodeTable =
      tree match {
