 ...
 * in the left subtree are smaller than the tweet at `b`. The eleemnts in the
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    ...
    if (left.empty)
      if (right.empty)
        elem
      else if (elem.retweets < rightMost.retweets)
        rightMost
      else
        elem
    // If left isn't empty
...
object GoogleVsApple {
  ...
  def themedTweets(wordList: List[String]): TweetSet =
    TweetReader.allTweets.filter(
      tweet => wordList.exists(word => tweet.text.contains(word))
    )

  lazy val googleTweets: TweetSet = themedTweets(google)
  lazy val appleTweets: TweetSet  = themedTweets(apple)
