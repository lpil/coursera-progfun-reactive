...
class ListsSuite extends FunSuite {
  ...
  test("Sum negative and positive numbers") {
    assert(sum(List(0,1,-1,2,-3)) === -1)
  }
  ...
  // Max of list function
  
  test("max throws NoSuchElementException on empty list") {
  ...
  test("max of a few numbers with max last") {
    assert(max(List(3,7,2,2,3,4,5,6,10)) === 10)
  }
