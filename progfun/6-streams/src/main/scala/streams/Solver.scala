...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(
    b: Block, history: List[Move]): Stream[(Block, List[Move])] =
      for {
        position <- b.legalNeighbors.toStream
      } yield (position._1, (position._2 :: history))
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
    explored: Set[Block]): Stream[(Block, List[Move])] =
      for {
        neighbor <- neighbors if !explored.contains(neighbor._1)
      } yield neighbor
   ...
   */
  def from(initial: Stream[(Block, List[Move])], explored: Set[Block]
      ...
      if (initial.isEmpty)
        ...
        val newNeighbors = newNeighborsOnly(
          neighborsWithHistory(block, moves), explored)
